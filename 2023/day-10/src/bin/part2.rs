...
fn main() {
    ...
    pipe_loop_pos.push(find_first_position_after_start(&pipe_loop_pos.first().unwrap(), &pipes_map).unwrap());
    ...
    loop {
        ...
        if pipe_loop_pos.last().unwrap() == start_pos { break; }
    }
...
fn count_enclosed_tiles(pipe_loop_pos: &Vec<Position>, pipes_map: &PipesMap) -> u32 {
    ...
    for y in 0..pipes_map.len() {
        ...
        for x in 0..pipes_map.first().unwrap().len() {
            let position: Position = Position{x: x as i32, y: y as i32};
