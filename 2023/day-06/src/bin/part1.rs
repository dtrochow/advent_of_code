...
fn main() {
    ...
    let times: Vec<u32> = lines[0].split(':').last().unwrap().split_whitespace()
                                                             .map(|s| s.parse().expect("Parsing error"))
                                                             .collect();
    let record_distances: Vec<u32> = lines[1].split(':').last().unwrap().split_whitespace()
                                                                        .map(|s| s.parse().expect("Parsing error"))
                                                                        .collect();
    ...
    println!("The product of numbers of ways is: {}", number_of_ways.iter().product::<u32>());
