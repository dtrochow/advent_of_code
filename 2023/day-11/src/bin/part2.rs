...
fn main() {
    ...
    for (index, pos) in galaxies_positions.iter().enumerate() {
        for i in (index + 1)..galaxies_positions.len() {
            distances_sum +=
                calculate_distance(pos, galaxies_positions.get(i).unwrap(), &galaxy) as u64;
        }
    }
...
fn calculate_distance(pos1: &Position, pos2: &Position, galaxy: &[String]) -> u32 {
    let x_expansion = get_x_expansion(pos1, pos2, galaxy);
    let y_expansion = get_y_expansion(pos1, pos2, galaxy);

    let x_distance: u32 = (pos2.x - pos1.x).unsigned_abs() - x_expansion + (x_expansion * 1000000);
    let y_distance: u32 = (pos2.y - pos1.y).unsigned_abs() - y_expansion + (y_expansion * 1000000);

    y_distance + x_distance
}

fn get_y_expansion(pos1: &Position, pos2: &Position, galaxy: &[String]) -> u32 {
    ...
    for index in start_y..(end_y + 1) {
        if galaxy[index as usize].starts_with('X') {
            y_exp_gaps_count += 1;
        }
    }
    y_exp_gaps_count
...
fn get_x_expansion(pos1: &Position, pos2: &Position, galaxy: &[String]) -> u32 {
...
fn find_galaxies_positions(lines: &[String]) -> Vec<Position> {
