...
fn compare_string_differences(string1: &String, string2: &String) -> u32 {
...
fn is_horizontal_smudge_mirror(pattern: &Pattern, horizontal_index: usize) -> bool {
    ...
    let mut smudge_count: u32 = 0;
    for i in 0..lines_to_check {
        smudge_count += compare_string_differences(&pattern[horizontal_index - i], &pattern[horizontal_index + i + 1]);
    }
    smudge_count == 1
}
