...
fn is_report_with_tolerance_safe(report: Report) -> bool {
    for i in 0..report.len() {
        let mut single_bad_report = report.clone();
        single_bad_report.remove(i);
        if is_report_safe(single_bad_report) {
            return true;
        }
    }
...
fn main() {
    ...
    for report in reports {
        if is_report_with_tolerance_safe(report) {
            safe_count += 1;
        }
    }
