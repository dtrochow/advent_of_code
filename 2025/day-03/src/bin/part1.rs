...
fn find_tens_digit(battery_bank: &BatteryBank) -> (u32, Vec<usize>) {
    let mut max = 0;
    for i in (0..(battery_bank.len() - 1)).rev() {
    ...
    for (i, bank) in battery_bank.iter().enumerate() {
        if *bank == max {
            indexes.push(i);
        }
    }
...
fn find_max_units_digit(battery_bank: &BatteryBank, tens_indexes: Vec<usize>) -> u32 {
    let mut max = 0;
    for tens_index in tens_indexes {
...
fn get_largest_joltage(battery_bank: &BatteryBank) -> u32 {
    let (tens, tens_indexes) = find_tens_digit(battery_bank);
    let units = find_max_units_digit(battery_bank, tens_indexes);
